Vue.component('checkout-view', {
  props: ['user', 'cart', 'total', 'gst', 'grandTotal', 'isLoggedIn'],
  ...
  template: `
    <div class="row my-5 justify-content-center">
      <div class="col-12 col-lg-10 col-xl-8">
        <div>
          <h5 class="text-center mb-3">{{ isLoggedIn ? 'Order Confirmation' : 'Please Login to Checkout' }}</h5>

          <!-- Show info when user is logged in -->
          <div v-if="isLoggedIn">
            <div class="mb-3">
              <h6>Shipping to:</h6>
          ...
          <div class="d-grid gap-2 col-12 col-md-3 mx-auto">
            <custom-button v-if="isLoggedIn" variant="primary-pink" @click="processOrder" block>Complete Order</custom-button>
            <custom-button v-if="!isLoggedIn" variant="primary-pink" @click="goToLogin" block>Login to Complete Order</custom-button>
            <custom-button variant="outline-secondary-dark" @click="goToCart" block>Back to Cart</custom-button>

            <!-- Show login link only user when not logged in -->
